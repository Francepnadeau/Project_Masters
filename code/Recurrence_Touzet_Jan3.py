...
K_dist=1  #maximal distance allowed between edit scripts
...
gen_state_DULA(0)  #We need to generate the tree once for the rest of the algorithm.
...
for state in DULA_STATES[1:]:
    for bit in possible_bits:
        next_state=DULA_transition(state,bit) #We compute the state after transition in DULA(k), starting at 'state' with the transition using 'bit'.
    ...
    for j in I_order:
        ...
        if j>=1 and j<=m+K_dist:  #We take j as an index for the output column, not the input as presented in the paper.
            for q in range(0,len(DULA_order)):   #We use q forto go through the states of DULA(k)
                print('q',q)
                for u in encod_letters(Sigma,P)[j-1]:  # u is the bit vector for each transition in the automaton.
                    print('u',u)
