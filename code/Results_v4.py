...
for w in words[0:len(words)/2]:
    ...
    #output_file.write('Words that repeat:'+" " +str(", ".join(repr(e) for e in repeat ))+"\n")
    
    no_rep=final_no_repeats(w,d,pre,Sigma,[],[])
    output_file.write('Myers bound without repeats:'+" "+str(len(no_rep))+"\n")
    
    pref=prefixes(w,d,pre,Sigma,[],[])
    output_file.write('Number of prefixes:'+" "+str(len(pref))+"\n")
    output_file.write('The prefixes are:'+" " +str(", ".join(pref))+"\n")
